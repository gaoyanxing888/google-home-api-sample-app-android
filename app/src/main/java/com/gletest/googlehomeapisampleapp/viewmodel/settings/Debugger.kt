...
package com.gletest.googlehomeapisampleapp.viewmodel.settings
...
@Suppress("UNUSED_VARIABLE", "unused", "ConstantConditionIf")
class Debugger private constructor(homeClientRef: HomeClient) : ViewModel() {
        ...
        node.apply {
          ...
          this.parameters.forEach { parameter ->
            ...
            parameter.apply {
              ...
              Log.d(
                TAG,
                "====== Dump automationGraph Starter.param.first.tag: ${this.param.first.tag}"
              )
              Log.d(
                TAG,
                "====== Dump automationGraph Starter.param.first.typeEnum: ${this.param.first.typeEnum}"
              )
              Log.d(
                TAG,
                "====== Dump automationGraph Starter.param.first.typeName: ${this.param.first.typeName}"
              )
              Log.d(
                TAG,
                "====== Dump automationGraph Starter.param.first.descriptor: ${this.param.first.descriptor}"
              )
  ...
  private suspend fun dumpAutomation(automation: Automation) {
    ...
    if (automationGraphSequentialFlow != null) {
      Log.d(
        TAG,
        "==== Dump automation.automationGraph.nodeId: ${automationGraphSequentialFlow.nodeId}"
      )
  ...
  private fun dumpTrait(deviceType: DeviceType, trait: Trait) {
    ...
    Log.d(
      TAG,
      "====== Dump Trait metadata.sourceConnectivity: ${trait.metadata.sourceConnectivity}"
    )
  ...
  private fun dumpDeviceType(deviceType: DeviceType) {
    ...
    deviceType.apply {
      ...
      Log.d(
        TAG,
        "====== Dump DeviceType metadata.sourceConnectivity: ${this.metadata.sourceConnectivity}"
      )
    }
  ...
  suspend fun dumpStructure(structureName: String) {
    homeClient.structures().list().filter { structure ->
      Log.i(
        TAG,
        "== Dump Structure: structure.name = ${structure.name} , structureName = $structureName"
      )
      structure.name == structureName
    }.forEach { structure ->
